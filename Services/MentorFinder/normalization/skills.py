...
student_skills = [" Python ", "ML", "machine learning", "JS", "web-dev "]       
alumni_skills = [
    [" C++ ", "Java", "deep learning", "React", "web-dev "],
    ["Python", "Machine Learning", "Data Science", "AI", "JavaScript"],
    ["Ruby", "Rails", "PostgreSQL", "Docker", "Java"],
    ["Java", "Spring Boot", "MySQL", "AWS", "Node.js"]
] 

def handle_skills_normalization(student_skills, alumni_skills):
    ...
    arr = []
    for alumni in alumni_skills:
        arr.append(normalize_skills(alumni))

    def get_skills_string(skills):
        return ', '.join(skills)

    student_skills = get_skills_string(student_skills)
    arr = [get_skills_string(skills) for skills in arr]

    return { "student_skills": student_skills, "alumni_skills": arr }
