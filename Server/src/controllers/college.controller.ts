...
export const handleFetchAllColleges = asyncHandler(
    ...
    res
      ...
      .json(
        new APIResponse(
          200,
          colleges || [],
          "Successfully fetched all colleges"
        )
      );
  }
);

export const handleAddCollege = asyncHandler(
  async (req: Request, res: Response) => {
    const {
      name,
      emailDomain,
      address,
      logo,
    }: { name: string; emailDomain: string; address: string; logo: string } =
      req.body;
...
export const handleRemoveCollege = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const publicId = college?.logo?.split("/")?.pop()?.split(".")[0];
    if (publicId) {
...
export const handleEditCollege = asyncHandler(
  async (req: Request, res: Response) => {
    const {
      name,
      emailDomain,
    ...
    const college = await College.findByIdAndUpdate(collegeId, payload, {
      new: true,
    });
